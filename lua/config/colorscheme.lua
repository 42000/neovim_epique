...
-- copié de BreadOnPenguins, un peu inutile mais rigolo
...
_G.load_theme = function()
	...
	if file then
        ...
        vim.cmd("set background=" .. file:read("*l"))
	file:close() end
end
...
_G.switch_theme = function()
	...
	local colorscheme, light = unpack(themes[current_theme_index])
	vim.cmd("colorscheme " .. colorscheme)
	vim.cmd("set background=" .. light)
	local file = io.open(theme_file, "w")
	if file then file:write(colorscheme .. "\n" .. light) file:close() end
end
